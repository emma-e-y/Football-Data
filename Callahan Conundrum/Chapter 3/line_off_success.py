...
# ROLLING SMOOTHING
# -----------------------------
df["off_sr_roll3"] = df["off_success_rate_pct"].rolling(3).mean()
df["off_sr_roll5"] = df["off_success_rate_pct"].rolling(5).mean()
...
sns.lineplot(data=df, x="season", y="off_success_rate_pct", color="grey", linewidth=0.8, zorder=1,
...
callahan_year = 2024
callahan_val = df.loc[df["season"] == 2024, "off_success_rate_pct"].values[0]
...
for year, label, dir_mult in events:
    val = df.loc[df["season"] == year, "off_success_rate_pct"].values[0]
...
# -----------------------------
ax.set_title(
    "Titans Offensive Success Rate (2000–2024)\n"
...
# -----------------------------
plt.tight_layout()
outpath = os.path.join(OUTDIR, "line_off_success_final.png")
...
df_sorted["yoy_change"] = df_sorted["off_success_rate_pct"].diff()

print("\nSEASON-BY-SEASON VALUES\n")
print(
    df_sorted[
        ["season", "off_success_rate_pct", "off_sr_roll3", "off_sr_roll5", "yoy_change"]
    ].round(4).to_string(index=False)
)

print("\nRANKINGS (BEST TO WORST)\n")
ranked = df_sorted.sort_values("off_success_rate_pct", ascending=False).reset_index(drop=True)
ranked["rank"] = ranked.index + 1
print(ranked[["rank", "season", "off_success_rate_pct"]].round(4).to_string(index=False))

print("\nANNOTATED SEASONS\n")
for yr in [2002, 2019, 2024]:
    r = df_sorted[df_sorted["season"] == yr].iloc[0]
    print(
        f"{yr}: value={r['off_success_rate_pct']:.4f}, "
