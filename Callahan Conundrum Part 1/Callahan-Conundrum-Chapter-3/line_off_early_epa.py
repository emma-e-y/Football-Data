...
# ROLLING SMOOTHING
# -----------------------------
df["off_sr_roll3"] = df["off_early_epa_per_play"].rolling(3).mean()
df["off_sr_roll5"] = df["off_early_epa_per_play"].rolling(5).mean()
...
sns.lineplot(data=df, x="season", y="off_early_epa_per_play", color="grey", linewidth=0.8, zorder=1,
    label="Offensive Early EPA"
)
...
callahan_year = 2024
callahan_val = df.loc[df["season"] == 2024, "off_early_epa_per_play"].values[0]
...
for year, label, dir_mult in events:
    val = df.loc[df["season"] == year, "off_early_epa_per_play"].values[0]

    # Only move Callahan arrow down to hit emoji corner
    if year == 2024:
        x_nudge = -0.25
        y_nudge = -0.006  # adjust until arrow hits bottom of emoji
    else:
        x_nudge = 0.0
        y_nudge = 0.0
    ...
    ax.annotate(
        label,
        xy=(year + x_nudge, val + y_nudge),
        xytext=(year + x_nudge - dx_years, val + y_nudge + dir_mult * dy_units),
...
# -----------------------------
ax.set_title(
    "Titans Offensive Early EPA/Play (2000–2024)\n"
...
ax.set_ylabel("Offensive Early EPA", fontsize=12)
...
# -----------------------------
plt.tight_layout()
outpath = os.path.join(OUTDIR, "line_off_early epa_final.png")
...
df_sorted["yoy_change"] = df_sorted["off_early_epa_per_play"].diff()

print("\nSEASON-BY-SEASON VALUES\n")
print(
    df_sorted[
        ["season", "off_early_epa_per_play", "off_sr_roll3", "off_sr_roll5", "yoy_change"]
    ].round(4).to_string(index=False)
)

print("\nRANKINGS (BEST TO WORST)\n")
ranked = df_sorted.sort_values("off_early_epa_per_play", ascending=False).reset_index(drop=True)
ranked["rank"] = ranked.index + 1
print(ranked[["rank", "season", "off_early_epa_per_play"]].round(4).to_string(index=False))

print("\nANNOTATED SEASONS\n")
for yr in [2002, 2019, 2024]:
    r = df_sorted[df_sorted["season"] == yr].iloc[0]
    print(
        f"{yr}: value={r['off_early_epa_per_play']:.4f}, "
        f"3Y={r['off_sr_roll3']:.4f}, "
        f"5Y={r['off_sr_roll5']:.4f}"
    )
