...
# ROLLING SMOOTHING
# -----------------------------
df["def_sr_roll3"] = df["def_early_epa_per_play"].rolling(3).mean()
df["def_sr_roll5"] = df["def_early_epa_per_play"].rolling(5).mean()
...
sns.lineplot(data=df, x="season", y="def_early_epa_per_play", color="grey", linewidth=0.8, zorder=1,
    label="Defensive Early EPA"
)
...
callahan_year = 2024
callahan_val = df.loc[df["season"] == 2024, "def_early_epa_per_play"].values[0]
...
for year, label, dir_mult in events:
    val = df.loc[df["season"] == year, "def_early_epa_per_play"].values[0]
...
# -----------------------------
ax.set_title(
    "Titans Defensive Early EPA/Play (2000–2024)\n"
...
ax.set_ylabel("Defensive Early EPA", fontsize=12)
...
# -----------------------------
plt.tight_layout()
outpath = os.path.join(OUTDIR, "line_def_early epa_final.png")
...
df_sorted["yoy_change"] = df_sorted["def_early_epa_per_play"].diff()

print("\nSEASON-BY-SEASON VALUES\n")
print(
    df_sorted[
        ["season", "def_early_epa_per_play", "def_sr_roll3", "def_sr_roll5", "yoy_change"]
    ].round(4).to_string(index=False)
)

print("\nRANKINGS (BEST TO WORST)\n")
ranked = df_sorted.sort_values("def_early_epa_per_play", ascending=False).reset_index(drop=True)
ranked["rank"] = ranked.index + 1
print(ranked[["rank", "season", "def_early_epa_per_play"]].round(4).to_string(index=False))

print("\nANNOTATED SEASONS\n")
for yr in [2002, 2019, 2024]:
    r = df_sorted[df_sorted["season"] == yr].iloc[0]
    print(
        f"{yr}: value={r['def_early_epa_per_play']:.4f}, "
