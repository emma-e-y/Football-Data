...
# ROLLING SMOOTHING
# -----------------------------
df["roll3"] = df["proe_pct_points"].rolling(3).mean()
df["roll5"] = df["proe_pct_points"].rolling(5).mean()
...
sns.lineplot(data=df, x="season", y="proe_pct_points", color="grey", linewidth=0.8, zorder=1,
    label="PROE"
)
...
callahan_year = 2024
callahan_val = df.loc[df["season"] == 2024, "proe_pct_points"].values[0]
...
for year, label, dir_mult in events:
    val = df.loc[df["season"] == year, "proe_pct_points"].values[0]
...
# -----------------------------
ax.set_title(
    "Titans Pass Rate Over Expected (2000–2024)\n"
...
ax.set_ylabel("PROE", fontsize=12)
...
# -----------------------------
plt.tight_layout()
outpath = os.path.join(OUTDIR, "line_proe_final.png")
...
df_sorted["yoy_change"] = df_sorted["proe_pct_points"].diff()

print("\nSEASON-BY-SEASON VALUES\n")
print(
    df_sorted[
        ["season", "proe_pct_points", "roll3", "roll5", "yoy_change"]
    ].round(4).to_string(index=False)
)

print("\nRANKINGS (BEST TO WORST)\n")
ranked = df_sorted.sort_values("proe_pct_points", ascending=False).reset_index(drop=True)
ranked["rank"] = ranked.index + 1
print(ranked[["rank", "season", "proe_pct_points"]].round(4).to_string(index=False))

print("\nANNOTATED SEASONS\n")
for yr in [2002, 2019, 2024]:
    r = df_sorted[df_sorted["season"] == yr].iloc[0]
    print(
        f"{yr}: value={r['proe_pct_points']:.4f}, "
        f"3Y={r['roll3']:.4f}, "
        f"5Y={r['roll5']:.4f}"
    )
